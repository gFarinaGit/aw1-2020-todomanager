  ...
  return new Promise((resolve, reject) => {
    ...
    switch (filter) {
      ...
      case "private":
        ...
        break;
      case "shared":
        sql = 'SELECT * FROM tasks WHERE private = 0';
        db.all(sql, [], (err, rows) => {
          if (err) {
            reject(err);
            return;
        ...
        break;
      case "today":
        sql = 'SELECT * FROM tasks WHERE DATE(deadline) = CURRENT_DATE;';
        db.all(sql, [], (err, rows) => {
          if (err) {
            reject(err);
            return;
        ...
        break;
      case "next7days":
        sql = 'SELECT * FROM tasks WHERE DATE(deadline) >= DATE(CURRENT_DATE, \'+1 DAY\') AND DATE(deadline) <= DATE(CURRENT_DATE, \'+7 DAY\');';
        db.all(sql, [], (err, rows) => {
          if (err) {
            reject(err);
            return;
